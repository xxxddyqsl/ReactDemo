    ...
    render() {
        return (
            <div className='app-assembly'>
                ...
                <div>
                    <div>
                        例子2-未使用 state + setState 更新状态：点击收藏按钮 收藏变成 取消收藏 只是修改text变量值 查看react是否会更新重新渲染dom
                    </div>
                    <button onClick={() => {
                        // 必须通过 setState 修改状态 通过React 更新
                        this.setState({
                            newText: (this.state.newText === '收藏' ? '取消收藏' : '收藏')
                        });
                ...
                <div>说明：this.state 是纯js对象，在vue中 date属性是利用Object.defineProperty处理过的，更改数据时会触发getter和setter，但是React中没有这样的处理，
                    如果直接更改的话，React是无法得知的，所以需要使用特殊的更改状态的方法setState。</div>
            </div>
        )
    }
}
