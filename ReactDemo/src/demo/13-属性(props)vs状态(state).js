  ...
  render() {
    return (
      <div>
        ...
        <button onClick={() => { this.props.text = '子组件-修改父组件传入的props值' }}>子组件-修改父组件传入的props值</button>
      </div>
    )
  }
}
...
state状态主要作用是用于组件保存、控制、修改自己的可变状态。state在组件内部初始化，可以被组件自身修改，而外部不能访问也不能修改，
...
props的主要作用是让使用该组件的父组件传入参数配置该组件，它是外部传进来的配置参数，组件内部无法控制也无法修改，除非外部主动传入新的props，否则组件的props永远保持不变。
